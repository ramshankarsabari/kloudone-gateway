...
export class SessionService {
  ...
  protected convertDateFromClient(session: ISession): ISession {
    const copy: ISession = Object.assign({}, session, {
      ...
      endDateTime: session.endDateTime && session.endDateTime.isValid() ? session.endDateTime.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      ...
      res.body.endDateTime = res.body.endDateTime ? moment(res.body.endDateTime) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((session: ISession) => {
        ...
        session.endDateTime = session.endDateTime ? moment(session.endDateTime) : undefined;
      });
    }
    return res;
  }
